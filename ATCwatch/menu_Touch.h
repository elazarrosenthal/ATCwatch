...
class TouchScreen : public Screen
{
  public:
    virtual void pre()
    {
      ...
      label_gesture = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_gesture, "Gesture: %02X", touch_data.gesture);
      lv_obj_align(label_gesture, LV_ALIGN_TOP_LEFT, 10, 25);
      label_touchpoints = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_touchpoints, "Gesture: %02X", touch_data.touchpoints);
      lv_obj_align(label_touchpoints,  LV_ALIGN_TOP_LEFT, 10, 45);
      label_event = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_event, "Event: %02X", touch_data.event);
      lv_obj_align(label_event,  LV_ALIGN_TOP_LEFT, 10, 65);
      label_xpos = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_xpos, "X: %02X", touch_data.xpos);
      lv_obj_align(label_xpos, LV_ALIGN_TOP_LEFT, 10, 85);
      label_ypos = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_ypos, "Y: %02X", touch_data.ypos);
      lv_obj_align(label_ypos, LV_ALIGN_TOP_LEFT, 10, 105);      
      label_versionInfo = lv_label_create(lv_scr_act());
      ...
      lv_obj_align_to(label_versionInfo, lv_scr_act(), LV_ALIGN_TOP_LEFT, 10, 125);
      label_int = lv_label_create(lv_scr_act());
      lv_label_set_text_fmt(label_int, "Interrupt: %i", digitalRead(TP_INT));  
      lv_obj_align_to(label_int, lv_scr_act(), LV_ALIGN_TOP_LEFT, 10, 145);
    }
