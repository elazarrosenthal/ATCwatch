...
class InfosScreen : public Screen
{
  public:
    virtual void pre()
    {
      ...
      label_version = lv_label_create(lv_scr_act());
      lv_label_set_text(label_version,"  Version:");
      lv_label_set_text_fmt(label_version, "  Version: %s",version_char);
      lv_obj_align_to(label_version, label, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);

      label_mac = lv_label_create(lv_scr_act());
      lv_label_set_text(label_mac,"  MAC:");
      lv_label_set_text_fmt(label_mac, "  MAC: %s",mac_char);
      lv_obj_align_to(label_mac, label_version, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 0);
    }
